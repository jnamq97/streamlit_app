    ...
    if mode == "Online":
        ...
        st.write(
            "실시간 보행 보조를 실행하는 모드입니다. 사용자의 카메라 영상을 기반으로 위험을 감지해 음성으로 경고를 줍니다.\n \
                1. select device를 통해 연결된 카메라를 선택해주세요.\n\
                2. start 버튼을 눌러 실시간 보행 보조를 실행합니다."
        )
        st.subheader("Offline mode란?")
        st.write(
            "보행 상황 이미지나 영상을 업로드하여 APAS의 성능을 시험해보는 모드입니다.\n\
                    1. data type을 image/video 중에 선택해주세요.\n \
                    2. browse file을 누르거나 드래그를 통해 테스트할 파일을 올려주세요.\n \
                    3. start infernce를 눌러 결과를 확인해보세요."
        )
